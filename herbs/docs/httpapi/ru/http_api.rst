...
НTTP-API не предназначено для изменения данных, а может использовать только для их чтения.
...
Одновременное задание полей при формировании запроса приводит к поисковому запросу типа "И": т.е. если задать одновременно `colstart=01.01.2016` и
...
Формирование запросов типа 'ИЛИ', учитывая сравнительно малые потенциально возможные объёмы базы данных (вряд ли стоит ожидать, что объем её превысит 10М записей), можно эмулировать  создав дополнительные запросы к базе.
...
Ответ сервера на поисковый запрос представляет собой `JSON-форматированный`_ текст, передаваемый по протоколу HTTP, и имеющий следующие поля:
